...
- PC 端与 Android 端同步更新，后续 release 将尽量保持两个端版本一致。
...
- iOS 端已开放 TestFlight 测试，但当前仍是 1.0 测试版，暂不包含慢动作、60 秒复盘和长视频骨架模式。
...
Android 1.3 支持短视频复盘和长视频骨架模式。

### HarmonyOS

HarmonyOS 端当前提供测试版 HAP 包，会随 GitHub Release 一起发布。
...
- 当前是 HarmonyOS 测试版安装包，不是 AppGallery 正式上架版本。
- 需要使用鸿蒙设备，并开启开发者模式 / USB 调试后通过 HDC 安装。
- 1.3 release 附件中会提供已签名 HAP 压缩包。

### iOS

iOS 端已开放 TestFlight 测试：
...
- 短视频复盘建议控制在 60 秒内，最好是一段连续回合或一个连续多拍片段。
- 长视频骨架模式可用于更长视频，但只生成全程骨架标注，不进行评分、重发力识别和证据分析。
